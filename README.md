...
- **単体 CLI ヘルパーは `scripts/<name>/`** に置きます（スキルにもエージェントにも属さない独立コマンド群。例: Apple Vision OCR バッチ `ocr-batch`、Mac GUI 操作 `mac-gui`）。実体はここ、`~/.claude/scripts/` へ symlink 配布はスキルと共通です。コンパイル済みバイナリは `.gitignore` でソースのみ追跡し、各自 `swiftc` 等でビルドします。
...
│   └── <name>.md
├── scripts/             # 単体 CLI ヘルパー（Vision OCR 等・各自ビルド）
│   └── <name>/          #   例 ocr-batch・mac-gui
...
単体 CLI ヘルパーは `~/.claude/scripts/` へ symlink します（バイナリは clone 後に各自ビルド）。

```bash
ln -s ~/Projects/agent-assets/scripts/<name> ~/.claude/scripts/<name>
```
