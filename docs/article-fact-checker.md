...
一次情報に辿れない主張は推測で判定せず「検証不能」として返します。草稿内の全 URL の死活確認（404 検出）も行います。

---

## 出力

主張ごとの判定表＋全体サマリを Markdown で返します。判定基準は「正: 一次情報と数値・表記・意味が完全一致」「疑: 部分一致・二次情報のみ・陳腐化の可能性」「誤: 一次情報と明確に矛盾」「検証不能: 一次情報に辿れない」の 4 区分で固定されており、信頼度は出典の質（高: 公式一次情報／中: 信頼できる二次情報／低: 状況証拠のみ）で決まります。

- **判定**: 正 / 疑 / 誤 / 検証不能（曖昧な「たぶん正しい」は出しません）
- **信頼度**: 高 / 中 / 低
- **出典**: URL ＋確認日
- **修正案**: 「誤」判定には新しい出典 URL 付きの修正文
- **リンク死活チェック表**: 草稿内の全 URL の HTTP ステータスと判定（OK / リンク切れ）
