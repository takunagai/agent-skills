...
> Codex など他のエージェントは `~/.agents/skills/deploy-cloudflare` を直接読みます。Claude Code は `~/.claude/skills/` を経由して同じ実体を参照します。
...
# ビルドチェックを省略（緊急時のみ）
/deploy-cloudflare production --skip-build
```
...
| コマンド | 動作 |
|----------|------|
| `/deploy-cloudflare` または `/deploy-cloudflare production` | 本番環境にデプロイ（`npm run deploy:production`） |
| `/deploy-cloudflare preview` | プレビュー環境にデプロイ（`npm run deploy:preview`） |
...
DEPLOY_PROJECT_DIR=~/Projects/my-next-app PRODUCTION_URL=https://example.com \
  bash ~/.agents/skills/deploy-cloudflare/scripts/deploy.sh production
```
...
| 対象構成 | 使うスキル |
|----------|-----------|
| Next.js + OpenNext | **deploy-cloudflare**（本スキル） |
