...
Google の画像生成モデル **Nano Banana** シリーズ（GA 版）— **Flash2 / Nano Banana 2**（推奨・万能型）、**Pro**（最高品質・テキスト精度最高）、**Lite / Nano Banana 2 Lite**（最速・最安・1K 専用）を使って、テキストからの画像生成・既存画像の編集・スタイルリファレンス・マルチターン反復修正を行う Claude Code スキルです。
...
- リファレンス画像からスタイル・色調・構図を転写（`-r`。入力画像 `-i` と合わせて全モデル最大14枚・合計7MBまで）
...
- 生成のたびに出力フォルダへ生成記録（`生成記録.md`）を自動作成（指示原文・使用プロンプト・料金目安・検証メモ）
...
| 速度 | 約5〜15秒 | 約15〜60秒 | 最速（sub-2秒） |
| 解像度 | 512px / 1K / 2K / 4K | 1K / 2K / 4K | 1K のみ |
| テキスト描画 | 良好 | 最高精度 | 基本 |
| 入力画像（内訳、合計は各14枚） | 物体10 + 人物4 | 物体6 + 人物5 + スタイル参照3 | 物体14 |
| スタイルリファレンス | 対応 | 対応 | 対応 |
| マルチターン編集 | 対応 | 対応 | 対応 |
...
### 料金の目安（画像1枚あたり, Standard tier, 2026-07-12 時点, 出典 https://ai.google.dev/gemini-api/docs/pricing）
...
**SDK は 2.11.0 以上が必須**です。本スキルは Interactions API（`client.interactions.create`）で画像を生成しており、2.11.0 は画像＋テキストの複数入力ブロックを確実に受け付ける最初のリリースだからです。

> **注**: macOS の Homebrew Python では `pip3 install -U --break-system-packages "google-genai>=2.11.0"` のように `--break-system-packages` が必要になる場合があります。

| パッケージ | 説明 |
|-----------|------|
| `google-genai`（>=2.11.0） | Google Gemini API の公式 Python SDK。Interactions API 経由の画像生成・テキスト生成に使用 |
...
リファレンス画像のスタイル・色調・構図を新しい画像に適用できます。**公式のスタイル参照スロットを持つのは Pro のみ**（最大3枚）で、スタイル転写は Pro が第一候補です。Flash2 と Lite にも参照画像は渡せます（`-i` と `-r` を合わせて全モデル共通で最大14枚・合計7MBまで）が、こちらは物体参照として扱われ、Pro のような専用のスタイル参照ではありません。
...
### マルチターン編集（段階的な修正、全モデル対応）

マルチターンの状態はサーバー側で管理されます（`previous_interaction_id` ベース）。セッションファイルは v2 形式で、各ターンの `interaction.id` を記録するだけのシンプルな構造です。旧形式（`generate_content` 世代、`history` キー方式）のセッションは継続できないため、`-c` で新規セッションを開始してください。継続はサーバー側の保持期間内のみ有効です（Paid Tier 55日 / Free Tier 1日）。期限切れのセッションを継続しようとすると 404 で失敗するので、その場合も `-c` で開始し直します。

継続ターンでのフラグの効き方は次のとおりです:

- **セッション開始時に固定される**（継続時に指定しても無視）: モデル `-m`、アスペクト比 `-a`、解像度 `-s`
- **毎ターン反映される**: 思考レベル `-t`、Google 検索 `-g`、Image Search `--image-search`
- **継続ターンでも渡せる**: 入力画像 `-i`、リファレンス画像 `-r`（そのターンの入力として送信されます）
- **使えない**: 枚数指定 `-N`（マルチターンは 1 ターン 1 枚。`-N 2` 以上を明示するとエラー、`config.json` の `num_images` は 1 に丸められます）

なお単発生成では `store=False` を送るためサーバー側に履歴は残りません。サーバー保持（Paid 55日 / Free 1日）が発生するのはマルチターン時だけです。
...
- `-n` を指定しない場合は `nanobanana_<タイムスタンプ>_1.png` のように自動命名されます
- `-N` のバリエーションは 2 枚目以降に `_v2`, `_v3` … が付きます（`-n city -N 4` → `city.png`, `city_v2.png`, `city_v3.png`, `city_v4.png`）
- マルチターン（`-c` / `--session`）ではターン番号 `_t1`, `_t2` … が付きます（`-n cabin -c` → `cabin_t1.png`、次のターンで `cabin_t2.png`）
- 出力先に同名ファイルがある場合は上書きせず `_2`, `_3` … の連番を付けて保存します（その旨のメッセージが出ます）

#### 生成記録（生成記録.md）

生成が完了するたびに、Claude が出力フォルダへ `生成記録.md` を自動作成します（既存なら日付付きセクションを追記。過去の記録は上書きしません）。記載内容:

- 生成日・モデル（正式名 + モデル ID）・設定（アスペクト比・解像度・実行形態）
- 料金目安（単価 × 枚数、tier と価格時点付き）
- ユーザーの指示の原文（引用）
- 各画像のファイル名 + 内容サマリ + 実際に使ったプロンプト全文（入力・参照画像があればそのパスも）
- 検証メモ（目視確認で気づいた破綻・文字化け等）

出力フォルダに残るのは画像と `生成記録.md`（マルチターン時は `session_*.json` も）だけです。実行時のログは出力フォルダには残しません。
...
| 引数 | 短縮 | デフォルト | 説明 |
|------|------|-----------|------|
| `--prompt` | `-p` | **必須** | テキストプロンプト（`--list-models` 時のみ省略可） |
| `--list-models` | | - | 利用可能モデル一覧を表示 |
| `--model` | `-m` | `flash2` | `flash2`（推奨）, `pro`, `lite` |
| `--input-image` | `-i` | なし | 編集用入力画像パス（複数指定可。`-r` と合計14枚・7MB まで） |
| `--reference` | `-r` | なし | スタイル/構図リファレンス画像パス（複数指定可。`-i` と合計14枚・7MB まで） |
| `--num-images` | `-N` | `1` | 生成する画像の枚数（最大10枚。マルチターン時は指定不可） |
...
| `--image-search` | | なし | Image Search 連携を有効化（flash2 のみ。`-g` と併用可） |
| `--chat` | `-c` | なし | マルチターン新規セッション（flash2/pro/lite） |
| `--session` | | なし | セッション JSON パス（既存セッションを継続。v2 形式のみ継続可、旧形式は `-c` で新規開始） |
| `--timeout` | | 120 | タイムアウト秒（4K は自動 420s。実際に API へ伝播） |

デフォルト列はビルトインの既定値です。`config.json` を置くと `model` / `aspect_ratio` / `output_dir` / `num_images` / `timeout` / `thinking_level` のデフォルトを上書きでき、CLI 引数はさらにそれより優先されます。
...
| `num_images` | int (1-10) | `1` | デフォルト生成枚数（マルチターン時は 1 に丸められます） |
...
必要なキーだけ記述すれば OK です。未指定のキーはビルトインデフォルトが使われます。表にないキーを書くと警告を出して無視され、JSON が壊れている場合も警告のうえビルトインデフォルトで続行します。
...
| 問題 | 解決策 |
|------|--------|
| API キーエラー | `echo $GEMINI_API_KEY` で設定を確認（`GOOGLE_API_KEY` でも可） |
| 権限エラー (403) | API キーが無効か、クォータ超過。[AI Studio](https://aistudio.google.com/apikey) でキーを確認 |
| 不正なリクエスト (400) | プロンプトとパラメータを確認（極端に長いプロンプト・不正な組み合わせなど） |
| モデルが見つからない (404) | `--list-models` でモデル ID を確認し、スクリプトの `MODEL_SPECS` を更新 |
| モデルが対応しない指定でエラー | `-s` / `-a` / `-g` / `--image-search` / `-t` はモデルごとに受理値が違う（例: lite は 1K のみ・検索連携なし、Image Search は flash2 のみ）。エラーメッセージに有効値が出る |
| 画像が生成されない | プロンプトが安全フィルターに引っかかった可能性。内容を変更して再試行（終了コード 2） |
| レート制限 (429) | 自動リトライ（最大3回、指数バックオフ）。頻発する場合は時間を空ける |
| `-N` の一部が失敗する | 失敗した枚だけ警告を出して残りを継続。全滅した場合のみ終了コード 2 |
| 4K がタイムアウト | `--timeout 600` で延長（デフォルトは 4K 時自動 420s） |
| 画像が多すぎる / 大きすぎる | `-i` と `-r` の合計は14枚まで、合計ファイルサイズは 7MB まで |
| `--chat` と `--session` を同時指定 | 排他。新規は `-c`、継続は `--session` のみを指定 |
| マルチターンで `-N` が使えない | 1 ターン 1 枚が原則。`-N 2` 以上を明示するとエラー（`config.json` の `num_images` は 1 に丸められる） |
| 旧形式セッションは継続できない | `history` キー方式（`generate_content` 世代）のセッションは非対応。`-c` で新規セッションを開始 |
| セッションファイルの形式が不正 | v2 形式のキー欠落・JSON 破損。`-c` で新規セッションを開始 |
| セッション継続時に 404 エラー | サーバー側の保持期限切れの可能性（Paid Tier 55日 / Free Tier 1日）。`-c` で新規セッションを開始 |
| パッケージ未インストール / Import error | `pip install -U "google-genai>=2.11.0" Pillow` |
| `python` が見つからない | macOS では `python3` を使用 |
| config.json が無視される | スキルディレクトリ直下（`SKILL.md` と同階層）に配置されているか確認。表にないキーは警告のうえ無視される |

終了コード: `0` 正常 / `1` 引数・環境エラー（API キー未設定、不正なパラメータ、ファイル不備など） / `2` 生成失敗（API エラー、画像が返らない） / `130` ユーザー中断（Ctrl-C）。
